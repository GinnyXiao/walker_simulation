...
struct PickMachine
{
    ...
    MoveGroup::Plan pickup_plan_1;
    MoveGroup::Plan pickup_plan_2;
...
auto to_cstring(PickState state) -> const char*
{
    ...
    case PickState::PlanFirstPickup:    return "PlanFirstPickup";
    case PickState::ExecuteFirstPickup: return "ExecuteFirstPickup";
    case PickState::WaitForSecondGoal:  return "WaitForSecondGoal";
    case PickState::PlanSecondPickup:   return "PlanSecondPickup";
    case PickState::ExecuteSecondPickup:return "ExecuteSecondPickup";
...
PickState DoWaitForGoal(PickMachine* mach)
{
    ...
    while (ros::ok()) {
        if (mach->goal_ready) {
            ...
            return PickState::PlanFirstPickup;
        }
    ...
    double* time_to_grasp)
{
    *conveyor_speed = object_vel.vector.y;
    ...
    grasp_pose_goal->pose.orientation.x = 0.0;
    grasp_pose_goal->pose.orientation.y = 0.0;
    grasp_pose_goal->pose.orientation.z = 0.7071068;
    grasp_pose_goal->pose.orientation.w = 0.7071068;
...
PickState DoPlanFirstPickup(PickMachine* mach)
{
    ROS_INFO("Plan link '%s' to the 1st grasp pose", mach->move_group->getEndEffectorLink().c_str());
    WaitForMoveGroup();

    g_move_group_busy = true;
    mach->move_group->setPoseTarget(mach->grasp_pose_goal.pose);
    auto err = mach->move_group->plan(mach->pickup_plan_1);
    ...
    return PickState::ExecuteFirstPickup;
}

void PublishPlannedPath(const MoveGroup::Plan* plan, int rate = 10, int iterations = 1000)
{
    int iteration = 0; 
    ros::Rate r(rate);
    while (ros::ok && iteration < iterations) {
        ...
        for (int i = 0; i < plan->trajectory_.joint_trajectory.points.size(); i++) {
            ...
            for (int j = 0; j < plan->trajectory_.joint_trajectory.points[i].positions.size(); j++) {
                robot_state.a.push_back(plan->trajectory_.joint_trajectory.points[i].positions[j]);
            }
...
PickState DoExecuteFirstPickup(PickMachine* mach)
{
    // publishing planed path
    PublishPlannedPath(&mach->pickup_plan_1);

#if 0
    g_move_group_busy = true;
    auto err = mach->move_group->execute(mach->pickup_plan_1);
    g_move_group_busy = false;

    if (err.val != moveit_msgs::MoveItErrorCodes::SUCCESS) {
        ROS_ERROR("Failed to move arm to the 1st grasp pose");
        mach->goal_ready = false;
        return PickState::WaitForGoal;
    }
#endif
    ...
    ROS_WARN("execution duration for 1st grasp is %f secs", execution_duration.toSec());

    return PickState::WaitForSecondGoal;
}

PickState DoWaitForSecondGoal(PickMachine* mach){
  bool is_second = false;
  geometry_msgs::PoseStamped curr_pose;
  while (ros::ok() && is_second == false) {
    for (auto& object : g_world_state.objects) {
      if (object.id == mach->claimed_id) {
        curr_pose = object.pose_estimates[object.pose_estimates.size() - 1].pose;
        if (curr_pose.pose.position.y < 1.7) {
          is_second = true;
          break;
        } 
      }
  ...
  float workspace_length = 0.2;
  float r = static_cast <float> (std::rand()) / (static_cast <float> (RAND_MAX / workspace_length));
  float fake_offset = r - workspace_length * 0.5;
  ROS_WARN("Plan for the second goal at offset: %f", fake_offset);
  mach->grasp_pose_goal.pose.position.x += fake_offset;
  mach->time_to_grasp = std::fabs((curr_pose.pose.position.y - mach->grasp_pose_goal.pose.position.y) / (mach->conveyor_speed));
  return PickState::PlanSecondPickup;
}

PickState DoPlanSecondPickup(PickMachine* mach)
{
    ROS_INFO("Plan link '%s' to the 2nd grasp pose", mach->move_group->getEndEffectorLink().c_str());
    WaitForMoveGroup();

    mach->time_at_execute_2 = ros::Time::now();

    g_move_group_busy = true;
    mach->move_group->setPoseTarget(mach->grasp_pose_goal.pose);
    auto err = mach->move_group->plan(mach->pickup_plan_2);
    g_move_group_busy = false;

    if (err.val != moveit_msgs::MoveItErrorCodes::SUCCESS) {
        ROS_ERROR("Failed to plan to the 2nd grasp pose");
        mach->goal_ready = false;
        return PickState::WaitForGoal;
    }

    return PickState::ExecuteSecondPickup;
}

PickState DoExecuteSecondPickup(PickMachine* mach)
{
    g_move_group_busy = true;
    auto err = mach->move_group->execute(mach->pickup_plan_2);
    g_move_group_busy = false;

    if (err.val != moveit_msgs::MoveItErrorCodes::SUCCESS) {
        ROS_ERROR("Failed to move arm to the 2nd grasp pose");
        mach->goal_ready = false;
        return PickState::WaitForGoal;
    }
...
PickState DoGraspObject(PickMachine* mach)
{
   ...
   // ROS_INFO("Wait an additional %f", buffer_time);
    ...
    ros::Duration execution_duration = ros::Time::now() - mach->time_at_execute_2;
    ROS_WARN("execution duration for 2nd grasp is %f secs", execution_duration.toSec());
...
int main(int argc, char* argv[])
{
    ...
    states[(int)PickState::PlanFirstPickup].pump = DoPlanFirstPickup;
    states[(int)PickState::ExecuteFirstPickup].pump = DoExecuteFirstPickup;
    states[(int)PickState::WaitForSecondGoal].pump = DoWaitForSecondGoal;
    states[(int)PickState::PlanSecondPickup].pump = DoPlanSecondPickup;
    states[(int)PickState::ExecuteSecondPickup].pump = DoExecuteSecondPickup;
