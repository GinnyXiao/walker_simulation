...
void PublishPlannedPath(const MoveGroupInterface::Plan* plan, int rate = 10, int iterations = 1000)
{
    int iteration = 0; 
    ros::Rate r(rate);
...
bool GetLastRobotState(PickMachine *mach, moveit_msgs::RobotState& state)
{
	return 1;
}
...
void Plan(PickMachine* mach)
{
	...
	if (plan_succeed)
	{
		PublishPlannedPath(&mach->current_plan);
	}
}
...
int main(int argc, char *argv[])
{
	...
	ros::Subscriber pose_sub = nh.subscribe("/Grasps", 1000, GraspPoseCallback);
